...
mod RESOURCE-ACCESS is
    ...
    op rsrc : Int -> Resource [ctor] .

    ops c1 c2 : Int -> Client [ctor] .

    vars N M : Int .

    crl [c1-consumes] : c1(N) rsrc(M) => c1(N + 1) rsrc(M - 1) if M > 0 . 
    crl [c2-consumes] : c2(N) rsrc(M) => c2(N + 1) rsrc(M - 1) if M > 0 . 
endm
...
mod RESOURCE-ACCESS-CHECK is
    ...
    op initial : -> Conf .
    eq initial = c1(0) c2(0) rsrc(3) .
endm

*** - is it possible that client 2 never gets access to the shared resource?
*** red modelCheck(initial, ~ [] c2-alive) .

*** - client 2 always gets access to the shared resource?
*** red modelCheck(initial, [] c2-alive) .
...
result ModelCheckResult: counterexample(
    {rsrc(3) c1(0) c2(0),'c1-consumes} 
    {rsrc(2) c1(1) c2(0),'c1-consumes}
    {rsrc(1) c1(2) c2(0),'c1-consumes},

    {rsrc(0) c1(3) c2(0),deadlock})
...
result ModelCheckResult: counterexample(
    {rsrc(3) c1(0) c2(0),'c1-consumes}
    {rsrc(2) c1(1) c2(0),'c1-consumes}
    {rsrc(1) c1(2) c2(0),'c2-consumes}, 

    {rsrc(0) c1(2) c2(1),deadlock})
